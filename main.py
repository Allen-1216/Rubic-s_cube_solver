...
def scan_color(): #讀取單顆顏色
    ...
    #cube3d[x][y] = ColorSensorS4.rgb() #RGB測試用
    ...
    if strcolor == Color.BLUE:
        ...
        cube3d[x][y] = 'D'
    #lego掃不到橘色，會把橘當成紅色，所以先把可辨認的顏色排除，留下紅色，再用RGB光反射作區別
    if cube3d[x][y] == 'R':
        if RGB[0] > 58: #隨環境不同數字需修正
            cube3d[x][y] = 'L'
        else:
            cube3d[x][y] = 'R'
    y += 1
...
def scan_plane(): #讀取一面的各個顏色
    ...
    motorC.run_angle(300, -230*3, then=Stop.HOLD, wait=True) #往前到中間 2
    scan_color()
    motorB.run_angle(300, -45*3, then=Stop.HOLD, wait=True) #底盤逆時針轉45度
    motorC.run_angle(300, 60*3, then=Stop.HOLD, wait=True) #後退掃描角落的 3
    scan_color()
    for r in range (0, 3):
        motorB.run_angle(300, -45*3, then=Stop.HOLD, wait=True)
        motorC.run_angle(300, -35*3, then=Stop.HOLD, wait=True) #前進掃描邊邊中間4、6、8
        scan_color()
        motorB.run_angle(300, -45*3, then=Stop.HOLD, wait=True)
        motorC.run_angle(300, 35*3, then=Stop.HOLD, wait=True) #後退掃描角落5、7、9
        scan_color()
    motorB.run_angle(300, -45*3, then=Stop.HOLD, wait=True)
    motorC.run_angle(300, -35*3, then=Stop.HOLD, wait=True) #前進掃描邊邊的中間 10
    scan_color()
...
#檢查用(中文方便辨識)
#cube3d = [['藍', '藍', '藍', '藍', '藍', '藍', '藍', '藍', '藍'], ['橘', '橘', '橘', '橘', '橘', '橘', '橘', '橘', '橘'], ['黃', '黃', '黃', '黃', '黃', '黃', '黃', '黃', '黃'], ['紅', '紅', '紅', '紅', '紅', '紅', '紅', '紅', '紅'], ['白', '白', '白', '白', '白', '白', '白', '白', '白'], ['綠', '綠', '綠', '綠', '綠', '綠', '綠', '綠', '綠']]
